...
const EmpresaSchema = object({
  ...
  cnpj: string()
    .matches(cnpjRegex, 'CNPJ inválido. Deve conter 14 números')
...
interface FormEmpresaProps {
  onSubmit?: (
    values: FormEmpresaValues,
    actions: FormikHelpers<FormEmpresaValues>
  ) => void
}
...
export default function FormEmpresa({ onSubmit }: FormEmpresaProps) {
  ...
  const handleEmpresaSubmit = async (
    ...
    try {
      // Aqui só salva no Firestore, sem criar usuário auth
      await setDoc(doc(firestore, 'empresas', values.cnpj), {
        nome: values.nome,
        cnpj: values.cnpj,
        email: values.email,
  ...
  return (
    <Box p={8} bg="gray.500" w="100%" mt="5%" mx="auto" borderRadius="md">
      ...
      <Formik
        ...
        onSubmit={onSubmit ?? handleEmpresaSubmit}
      >
        {({ isSubmitting }) => (
          <Form>
            <Stack spacing={4}>
              ...
              <Field name="cnpj">
                {({ field }: any) => (
                  <FormControl isRequired>
                    ...
                    <Input {...field} type="text" maxLength={14} bg="white" />
                    <Text color="red.300" fontSize="sm">
              ...
              <Field name="telefone">
                {({ field }: any) => (
                  <FormControl isRequired>
                    ...
                    <Input {...field} type="tel" bg="white" maxLength={11} />
                    <Text color="red.300" fontSize="sm">
